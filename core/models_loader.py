...
class Provider:
    def __init__(self, provider: str, env_key: str, api_base: str):
        ...
        logger.debug("Provider %s initialized (api_key: %s)", provider, "Set" if self.api_key else "Missing")
...
def _load_providers() -> Dict[str, Provider]:
    ...
    for p in providers_list:
        ...
        if name:
            providers[name] = Provider(p.get("provider"), p.get("env_key"), p.get("api_base"))
    return providers
